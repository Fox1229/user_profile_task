...
object MyJdbcUtil {
    ...
    val MYSQL_URL: String = properties.getProperty("mysql.url")
    ...
    def queryList[T <: AnyRef](sql: String, clazz: Class[T], underScoreToCamel: Boolean): List[T] = {
        ...
        while (rs.next()) {
            ...
            for (i <- 1 to md.getColumnCount) {
                ...
                MyBeanUtil.setV(obj, propertyName, rs.getObject(i))
    ...
    def queryOne[T <: AnyRef](sql: String, clazz: Class[T], underScoreToCamel: Boolean): Option[T] = {
        // println(sql)
        Class.forName("com.mysql.jdbc.Driver")
        val resultList: ListBuffer[T] = new ListBuffer[T]()
